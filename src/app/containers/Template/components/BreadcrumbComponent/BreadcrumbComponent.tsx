...
const BreadcrumbComponent = (): JSX.Element => {
  ...
  const setBreadcrumbItems = (path: string) => {
    ...
    if (pathNames.length > 0) {
      if (pathNames[1]) {
        setCategory(intl.formatMessage({ id: `page.name.${pathNames[1]}` }));
      }
      if (pathNames[2]) {
  ...
  return (
    <>
      <Breadcrumb style={{ margin: '16px 0' }}>
        ...
        <Breadcrumb.Item>
          <Button type="link" onClick={() => (window.location.href = `/${category}`)}>
            {category}
          </Button>
        </Breadcrumb.Item>
        {page && (
          <Breadcrumb.Item>
            <Button type="link" onClick={() => (window.location.href = `/${page}`)}>
              {page}
            </Button>
          </Breadcrumb.Item>
        )}
